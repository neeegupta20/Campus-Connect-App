...
app.get('/check-in-status',async(req,res)=>{
    try{
        const authHeader=req.headers['authorization'];
        const token=authHeader.split(' ')[1];
        if (!token){
        ...
        jwt.verify(token,'1234567890',async(err,tokenData)=>{
            ...
            if(!zoneId){
                return res.status(400).json({error:"Zone ID MISSING"});
            }
            const checkInRecord=await checkins.findOne({
                userId:tokenData.id,
                zoneId:zoneId,
            });
            res.status(200).json({checkedIn:!!checkInRecord});
        });
    }catch(error){
        res.status(500).json({error:"SERVER ERROR"});
    }
