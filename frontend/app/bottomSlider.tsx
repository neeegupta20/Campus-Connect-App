...
import ConnectZone from '../app/ConnectZones';
  ...
  const gesture = Gesture.Pan()
    ...
    .onEnd(() => {
      if (translateY.value > SCREEN_HEIGHT / 2) {
        translateY.value = withSpring(SCREEN_HEIGHT - 250); 
      } else {
        translateY.value = withSpring(-SCREEN_HEIGHT / 3); 
      }
    })
  useEffect(() => {
    translateY.value = withSpring(isOpen ? -SCREEN_HEIGHT / 3 : SCREEN_HEIGHT, { damping: 50 });
  }, [isOpen]);
